...
##	4. Calculate the average salary of all employees with gender "F"
SELECT AVG(s.salary) AS average_salary
FROM employees e
INNER JOIN salaries s ON e.emp_no = s.emp_no
WHERE e.gender = 'F';

##	5. Calculate the average salary of all employees with gender "M"
SELECT AVG(s.salary) AS average_salary
FROM employees e
INNER JOIN salaries s ON e.emp_no = s.emp_no
WHERE e.gender = 'M';
...
SELECT e.first_name, e.last_name, d.dept_name, MAX(s.salary) AS highest_salary
FROM employees e
...
select 
   ...
   sum(dept_emp.to_date - dept_emp.from_date) as totalWorkDuration
from dept_emp 
left join departments on dept_emp.dept_no = departments.dept_no
where dept_emp.emp_no = '10102'
group by dept_emp.dept_no, departments.dept_name
order by totalWorkDuration desc
limit 1;
